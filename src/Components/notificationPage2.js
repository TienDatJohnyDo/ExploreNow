...
const CollabBox = ({pic, user, msg, onClick, boxKey}) => {
    ...
    const handleAcceptClick = (key) => {
        onClick(key); // Remove the collab request box
    ...
    return (
      <div className="box">
        ...
        <button className="button button-2" onClick={() => handleAcceptClick(boxKey)}>
          Accept
        </button>
      </div>
      </div>
...
function NotifcationPage() {
  ...
  const [collabBoxes, setCollabBoxes] = useState([
    <CollabBox key={1} pic={laurenPic} user={"Lauren"} msg={"Paris Trip"} />,
    // Add more FollowBox components here
  ]);
  ...
  return (
    <div className="discover-container">
      <div className='collab-container'>
        <h1 className="title">Collab Requests 2</h1>
        <div className='box1'>
